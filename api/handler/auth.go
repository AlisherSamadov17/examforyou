...
func (h *Handler) CustomerLogin(c *gin.Context)  {
	...
	if err := check.ValidatePhoneNumberOfCustomer(loginReq.Login); !true {
		handlerResponseLog(c,h.Log,"Email address does not exist or is undeliverable"+loginReq.Login, http.StatusBadRequest, err)
		return
	}

	if err := check.ValidatePassword(loginReq.Password); !true {
		handlerResponseLog(c,h.Log,"Email address does not exist or is undeliverable"+loginReq.Password, http.StatusBadRequest, err)
		return
	}
...
func (h *Handler) CustomerRegisterConfirm(c *gin.Context) {
    ...
    if err := check.CheckEmail(req.Mail); err != nil {
		handlerResponseLog(c, h.Log, "error while validating email", http.StatusBadRequest, err.Error())
	  }

	confResp, err := h.Services.Auth().CustomerRegisterConfirm(c.Request.Context(), req)
	if err != nil {
		handlerResponseLog(c, h.Log, "error while confirming", http.StatusUnauthorized, err.Error())
		return
	}

	handlerResponseLog(c, h.Log, "Succes", http.StatusOK, confResp)
