...
func (h Handler) CreateCustomer(c *gin.Context) {
	...
	if err := check.ValidateGmailCustomer(customer.Mail); !true {
		handlerResponseLog(c, h.Log, "error while validating Email"+customer.Mail, http.StatusBadRequest, err)
		return
	}
...
func (h Handler) UpdateCustomerStatus(c *gin.Context) {
    ...
    if err != nil {
        handlerResponseLog(c, h.Log, "error while updating customer status", http.StatusInternalServerError, err.Error())
        return
    }
